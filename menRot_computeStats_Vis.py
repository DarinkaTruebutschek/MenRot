...
ListAnalysis = ['Train_AllSeen_Test_AllSeen']
...
#ListSubjects = ['lm130479', 'am150105', 'cb140229', 'nb140272', 'mj100109', 'dp150209', 
	#'ag150338', 'ml140071', 'rm080030', 'bl160191', 'lj150477', 'at140305', 
	#'df150397', 'rl130571', 'mm140137', 'mb160304', 'lk160274', 'av160302', 'cc150418', 
	#'cs150204', 'mp110340', 'lg160230', 'mp150285', 'ef160362', 'ml160216', 'pb160320', 
	#'cc130066', 'in110286', 'ss120102'] #1 subject didnt have enough unseen correct trials and so will have to be excluded in the Train_RotUnseenAcc condition
...
if ListFrequency[0] is 'all':
	...
	filename = '_Train_AllSeen_Test_AllSeen_' + ListFrequency[0]

tail = 1#0 = 2-sided, 1 = 1-sided
chance = 0.5 #for analyses involving 
...
for analysis in ListAnalysis:
	...
	if ListFrequency[0] is 'all':
		...
		np.save(res_path + '/' + analysis + '_' + ListFrequency[0] + '-time.npy', time)
		np.save(res_path + '/' + analysis + '_' + ListFrequency[0] + '-all_scores.npy', all_scores)
	...
	if (ListAnalysis[0] is not 'Loc_Trainloc_TestAllSeen') and (ListAnalysis[0] is not 'Resp_Trainloc_TestAllSeen') and (ListAnalysis[0] is not 'Infer_Trainloc_TestAllSeen'):
		if ListFrequency[0] is 'all':
			...
			np.save(res_path + '/' + analysis + '_' + stat_params + str(tail) + '_' + ListFrequency[0] + '-p_values.npy', p_values)
			np.save(res_path + '/' + analysis + '_' + stat_params + str(tail) + '_' + ListFrequency[0] + '-p_values_off.npy', p_values_off)
			np.save(res_path + '/' + analysis + '_' + stat_params + str(tail) + '_' + ListFrequency[0] + '-p_values_diag.npy', p_values_diag)
	else:
		np.save(res_path + '/' + analysis + '_' + stat_params + str(tail) + '-p_values.npy', p_values)
