...
def menRot_newPipeline_subscore_decLoc(wkdir, Condition, Subject, FileName, decCond):
	...
	from menRot_newPipeline_doRegression_subscore import menRot_newPipeline_doRegression_subscore
	from menRot_newPipeline_loc_cfg import (result_path)
	from menRot_newPipeline_loc_cfg import grouped, n_folds
	...
	def menRot_prepReg_loc(wkdir, Condition, Subject, FileName, decCond):
		...
		from menRot_newPipeline_loc_cfg import (data_path, baseline, baseline_loc, downsampling, trainTime, testTime, prediction_method, probabilities, scorer, featureSelection, loc, n_folds, baselineCorr, frequency, selChannels)
		...
		if frequency[0] is 'all':
			...
			#Select only subset of channels if needed
			if np.shape(selChannels)[0] > 1:
				ch_names = epoch.info['ch_names']
				selection = []

				for sensors in ch_names:
					for subselection in selChannels:
						if subselection in sensors:
							print(sensors)
							selection.append(sensors)

				picks = mne.pick_channels(ch_names=ch_names, include=selection)
				epoch = epoch.pick_channels([epoch.ch_names[pick] for pick in picks])
		else: 
		...
		if decCond is 'loc':
			...
			label = 'respAngle'
			label_test = 'respAngle'
			label_loc = 'angle'
		elif decCond is 'infer':
			label = 'inferAngle'
			label_test = 'inferAngle'
			label_loc = 'angle'
		elif decCond is 'locInfer':
			label = 'angle'
			label_test = 'inferAngle'
		elif decCond is 'locResp':
			label = 'angle'
			label_test = 'respAngle'
		elif decCond is 'inferLoc':
			label = 'inferAngle'
			label_test = 'angle'
		elif decCond is 'inferResp':
			label = 'inferAngle'
			label_test = 'respAngle'
		elif decCond is 'respLoc':
			label = 'respAngle'
			label_test = 'angle'
		elif decCond is 'respInfer':
			label = 'respAngle'
			label_test = 'inferAngle'
		...
		if trainset is 'Train_loc':
			...
			if testset is 'Test_rot':
				...
				y_test = np.ravel(np.array(list(info[label_test])))
			...
			if testset is 'Test_Rot':
				...
				X_test = epoch[sel2]
				y_test = np.ravel(np.array(list(info[sel2][label_test])))
			...
			if testset is 'Test_AllUnseenCorr':
				...
				X_test = epoch[sel3]
				y_test = np.ravel(np.array(list(info[sel3][label_test])))
		...
		####################################################################
		#Decoding
		(gat, score_seen, score_unseen, score_corr, score_incorr, 
		...
		label_rot, label_noRot, label_rot_seen_corr, label_noRot_seen_corr) = menRot_newPipeline_doRegression_subscore(X_train, y_train, X_test, y_test, params, 
	...
	if (grouped and n_folds == 5):
		myFolder = '/Grouped/FiveFolds/Subscore/FeatureSelection/'
	elif (grouped and n_folds == 4):
		myFolder = '/Grouped/FourFolds/Subscore/FeatureSelection/'
	elif (grouped and n_folds == 8):
		myFolder = '/Grouped/EightFolds/Subscore/FeatureSelection/'
	elif (grouped and n_folds == 10):
		myFolder = '/Grouped/TenFolds/Subscore2/FeatureSelection/'
	elif (grouped and n_folds == 2):
		myFolder = '/Grouped/TwoFolds/Subscore/FeatureSelection/'
	else:
		myFolder = '/'
	
	#Save data
	fname = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-params'
	np.save(fname, params)
	
	fname1 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-time'
	np.save(fname1, time)
	
	fname2 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] +'-gat'
	np.save(fname2, gat)
	
	fname3 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreSeen'
	np.save(fname3, score_seen)
	
	fname4 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreUnseen'
	np.save(fname4, score_unseen)
	
	fname5 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreUnseenCorr'
	np.save(fname5, score_corr)
	
	fname6 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreUnseenIncorr'
	np.save(fname6, score_incorr)
	
	
	fname7 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRotSeen'
	np.save(fname7, score_rot_seen)
	
	fname8 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRotUnseen'
	np.save(fname8, score_rot_unseen)
	
	fname9 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRotUnseenCorr'
	np.save(fname9, score_rot_corr)
	
	fname10 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRotUnseenIncorr'
	np.save(fname10, score_rot_incorr)
	
	
	fname11 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRotSeen'
	np.save(fname11, score_noRot_seen)
	
	fname12 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRotUnseen'
	np.save(fname12, score_noRot_unseen)
	
	fname13 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRotUnseenCorr'
	np.save(fname13, score_noRot_corr)
	
	fname14 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRotUnseenIncorr'
	np.save(fname14, score_noRot_incorr)
	
	
	fname15 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreLeftSeen'
	np.save(fname15, score_left_seen)
	
	fname16 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreLeftUnseen'
	np.save(fname16, score_left_unseen)
	
	fname17 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreLeftUnseenCorr'
	np.save(fname17, score_left_corr)
	
	fname18 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreLeftUnseenIncorr'
	np.save(fname18, score_left_incorr)
	
	
	fname19 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRightSeen'
	np.save(fname19, score_right_seen)
	
	fname20 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRightUnseen'
	np.save(fname20, score_right_unseen)
	
	fname21 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRightUnseenCorr'
	np.save(fname21, score_right_corr)
	
	fname22 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRightUnseenIncorr'
	np.save(fname22, score_right_incorr)
	
	
	fname23 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelSeen'
	np.save(fname23, label_seen)
	
	fname24 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelUnseen'
	np.save(fname24, label_unseen)
	
	fname25 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelUnseenCorr'
	np.save(fname25, label_corr)
	
	fname26 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelUnseenIncorr'
	np.save(fname26, label_incorr)
	
	
	fname27 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRotSeen'
	np.save(fname27, label_rot_seen)
	
	fname28 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRotUnseen'
	np.save(fname28, label_rot_unseen)
	
	fname29 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRotUnseenCorr'
	np.save(fname29, label_rot_corr)
	
	fname30 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRotUnseenIncorr'
	np.save(fname30, label_rot_incorr)
	
	
	fname31 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelNoRotSeen'
	np.save(fname31, label_noRot_seen)
	
	fname32 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelNoRotUnseen'
	np.save(fname32, label_noRot_unseen)
	
	fname33 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelNoRotUnseenCorr'
	np.save(fname33, label_noRot_corr)
	
	fname34 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelNoRotUnseenIncorr'
	np.save(fname34, label_noRot_incorr)
	
	
	fname35 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelLeftSeen'
	np.save(fname35, label_left_seen)
	
	fname36 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelLeftUnseen'
	np.save(fname36, label_left_unseen)
	
	fname37 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelLeftUnseenCorr'
	np.save(fname37, label_left_corr)
	
	fname38 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelLeftUnseenIncorr'
	np.save(fname38, label_left_incorr)
	
	
	
	fname39 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRightSeen'
	np.save(fname39, label_right_seen)
	
	fname40 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRightUnseen'
	np.save(fname40, label_right_unseen)
	
	fname41 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRightUnseenCorr'
	np.save(fname41, label_right_corr)
	
	fname42 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRightUnseenIncorr'
	np.save(fname42, label_right_incorr)
	
	
	fname43 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRot'
	np.save(fname43, label_rot)
	
	fname44 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelNoRot'
	np.save(fname44, label_noRot)
	
	fname45 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-labelRotSeenCorr'
	np.save(fname45, label_rot_seen_corr)
	
	fname46 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] +  frequency[0] + '-labelNoRotSeenCorr'
	np.save(fname46, label_noRot_seen_corr)
	
	
	fname47 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRot'
	np.save(fname47, score_rot)
	
	fname48 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRot'
	np.save(fname48, score_noRot)
	
	fname49= result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreRotSeenCorr'
	np.save(fname49, score_rot_seen_corr)
	
	fname50 = result_path + '/newPipeline/' + myFolder + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + frequency[0] + '-scoreNoRotSeenCorr'
