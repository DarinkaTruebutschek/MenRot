...
ListCondition = [['Train_AllUnseen', 'Test_AllUnseen']]
...
initbody = initbody + 'import menRot_newPipeline_quadrants_decLoc as DecInd\n'
...
for d, decCond in enumerate(ListDecCond):
	for c, condcouple in enumerate(ListCondition):
		for s, subject in enumerate(ListSubject):
			...
			body = initbody + "DecInd.menRot_newPipeline_quadrants_decLoc('" + wkdir + "',"
