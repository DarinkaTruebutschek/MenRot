...
from menRot_newPipeline_loc_cfg_Riemann import (wkdir)
...
ListCondition = [['Train_noRot', 'Test_noRot']]

#ListDecCond = ['loc', 'loc', 'loc', 'loc', 
	#'resp', 'resp', 'resp', 'resp',
	#'infer', 'infer', 'infer', 'infer']

ListDecCond = ['resp', 'infer']
...
initbody = initbody + 'import menRot_newPipeline_subscore_decLoc_Riemann as DecInd\n'
...
for d, decCond in enumerate(ListDecCond):
	for c, condcouple in enumerate(ListCondition):
		for s, subject in enumerate(ListSubject):
			...
			body = initbody + "DecInd.menRot_newPipeline_subscore_decLoc_Riemann('" + wkdir + "',"
...
somaWF_name = os.path.join(wkdir, 'SomaWF/Decoding/menRot_decLoc_workflow_RiemannLonger')
