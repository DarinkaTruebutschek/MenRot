...
def menRot_newPipeline_doRegression_subscore_EOG(X_train, y_train, X_test, y_test, params, 
...
result_path, FileName, Subject, Condition):
	...
	gat.picks = np.array([0,1]) #otherwise, only data considered will be actual MEG channels
	#gat = GeneralizingEstimator(clf, scoring = scorer, n_jobs = 8)
	
	gat.fit(X_train, y = y_train)
	
	#Save gat as pickle
	fsave = result_path + '/newPipeline/Grouped/FiveFolds/IntermediateLocations/EOG/' + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + params['freq'] + '-gatForWeights.p'
	...
	y_pred = gat.y_pred_
	fsave = result_path + '/newPipeline/Grouped/FiveFolds/IntermediateLocations/EOG/' + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + params['freq'] + '-y_pred.p'
	pickle.dump(y_pred, open(fsave, "wb"))
	#scores = gat.score()
	
	#Save gat as pickle
	fsave = result_path + '/newPipeline/Grouped/FiveFolds/IntermediateLocations/EOG/' + FileName + '/IndRes/' + Subject + '_' + Condition[0] + '_' + Condition[1] + params['freq'] + '-gatForDis.p'
	pickle.dump(gat, open(fsave, "wb"))
