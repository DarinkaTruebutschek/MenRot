...
def menRot_newPipeline_subscore_decLoc_EOG(wkdir, Condition, Subject, FileName, decCond):
	...
	from fldtrp2mne_EOG import fldtrp2mne_EOG
	from menRot_newPipeline_doRegression_subscore_EOG import menRot_newPipeline_doRegression_subscore_EOG
	...
	def menRot_prepReg_loc(wkdir, Condition, Subject, FileName, decCond):
		...
		from menRot_newPipeline_loc_cfg import (data_path, baseline, baseline_loc, downsampling, trainTime, testTime, prediction_method, probabilities, scorer, featureSelection, loc, n_folds, baselineCorr, frequency)
		...
		if frequency[0] is 'all':
			...
			epoch = fldtrp2mne_EOG(fname, 'data')
			epoch.info['lowpass'] = float(30) #to correct lowpass filter info
			...
			epoch = fldtrp2mne_EOG(fname, 'freqDec')
		...
		label_rot, label_noRot, label_rot_seen_corr, label_noRot_seen_corr) = menRot_newPipeline_doRegression_subscore_EOG(X_train, y_train, X_test, y_test, params, 
	...
	if (grouped and n_folds == 5):
		myFolder = '/Grouped/FiveFolds/IntermediateLocations/EOG/'
	elif (grouped and n_folds == 4):
		myFolder = '/Grouped/FourFolds/IntermediateLocations/EOG/'
	elif (grouped and n_folds == 8):
		myFolder = '/Grouped/EightFolds/IntermediateLocations/EOG/'
	elif (grouped and n_folds == 10):
		myFolder = '/Grouped/TenFolds/Subscore2/EOG/'
	elif (grouped and n_folds == 2):
		myFolder = '/Grouped/TwoFolds/IntermediateLocations/EOG/'
	else:
		myFolder = '/'
