...
ListCondition = [['Train_Rot', 'Test_Rot'], ['Train_noRot', 'Test_noRot']]

#ListDecCond = ['loc', 'loc', 'loc', 'loc', 
	#'resp', 'resp', 'resp', 'resp',
	#'infer', 'infer', 'infer', 'infer']

ListDecCond = ['infer', 'resp', 'loc']
...
initbody = initbody + 'import menRot_newPipeline_decLoc as DecInd\n'
...
for d, decCond in enumerate(ListDecCond):
	for c, condcouple in enumerate(ListCondition):
		for s, subject in enumerate(ListSubject):
			...
			body = initbody + "DecInd.menRot_newPipeline_decLoc('" + wkdir + "',"
