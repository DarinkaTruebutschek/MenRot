...
ListCondition = [['Train_AllSeen', 'Test_AllSeen']]

#ListDecCond = ['loc', 'loc', 'loc', 'loc', 
	#'resp', 'resp', 'resp', 'resp',
	#'infer', 'infer', 'infer', 'infer']

ListDecCond = ['loc', 'resp', 'infer']
...
initbody = initbody + 'import menRot_updated_decLoc as DecInd\n'
...
for d, decCond in enumerate(ListDecCond):
	for c, condcouple in enumerate(ListCondition):
		for s, subject in enumerate(ListSubject):
			...
			body = initbody + "DecInd.menRot_updated_decLoc('" + wkdir + "',"
