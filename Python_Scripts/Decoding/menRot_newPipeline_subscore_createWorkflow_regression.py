...
initbody = initbody + 'import menRot_newPipeline_subscore_decLoc as DecInd\n'
...
for d, decCond in enumerate(ListDecCond):
	for c, condcouple in enumerate(ListCondition):
		for s, subject in enumerate(ListSubject):
			...
			body = initbody + "DecInd.menRot_newPipeline_subscore_decLoc('" + wkdir + "',"
			...
			name_file = []
			name_file = os.path.join(wkdir, ('SomaWF/Decoding/Jobs/jobs_featureSelection_' + jobname + '.py'))
...
somaWF_name = os.path.join(wkdir, 'SomaWF/Decoding/menRot_decLoc_workflow_featureSelection')
