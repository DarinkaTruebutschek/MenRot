...
def computePredictedAngles(my_analysis, my_visibility, my_bins, my_split):
	...
	if ListAnalysis[0] is not 'InferResp_TrainAll_TestAll':
		path = '/neurospin/meg/meg_tmp/MenRot_Truebutschek_2016/Decoding/newPipeline/Grouped/TenFolds/Subscore2/'
		res_path = '/neurospin/meg/meg_tmp/MenRot_Truebutschek_2016/Decoding/newPipeline/Grouped/TenFolds/IntermediateLocations/' + ListAnalysis[0] + '/GroupRes/Stats/'
	else:
	...
	for anali, analysis in enumerate(ListAnalysis):
		for subi, subject in enumerate(ListSubjects):
			...
			filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-y_pred.p'
			...
			if ListSelection is 'all':
				...
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelSeen.npy'
				sel = np.load(filename)
			elif ListSelection is 'unseen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelUnseen.npy'
				sel = np.load(filename)
			elif ListSelection is 'RotSeen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRotSeen.npy'
				sel = np.load(filename)
			elif ListSelection is 'RotUnseen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRotUnseen.npy'
				sel = np.load(filename)
			elif ListSelection is 'NoRotSeen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelNoRotSeen.npy'
				sel = np.load(filename)
			elif ListSelection is 'NoRotUnseen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelNoRotUnseen.npy'
				sel = np.load(filename)
			elif ListSelection is 'NoRotUnseenCorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelNoRotUnseenCorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'NoRotUnseenIncorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelNoRotUnseenIncorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'LeftSeen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftSeen.npy'
				sel = np.load(filename)
			elif ListSelection is 'RightSeen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightSeen.npy'
				sel = np.load(filename)
			elif ListSelection is 'NoRotSeenCorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelNoRotSeenCorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'LeftUnseen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseen.npy'
				sel = np.load(filename)
			elif ListSelection is 'RightUnseen':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseen.npy'
				sel = np.load(filename)
			elif ListSelection is 'RightUnseenCorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseenCorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'LeftUnseenCorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseenCorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'RightUnseenIncorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseenIncorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'LeftUnseenIncorr':
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseenIncorr.npy'
				sel = np.load(filename)
			elif ListSelection is 'BothDirSeen':
				filename1 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftSeen.npy'
				sel1 = np.load(filename1)
				filename2 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightSeen.npy'
				sel2 = np.load(filename2)
			elif ListSelection is 'BothDirUnseen':
				filename1 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseen.npy'
				sel1 = np.load(filename1)
				filename2 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseen.npy'
				sel2 = np.load(filename2)
			elif ListSelection is 'BothDirUnseenCorr':
				filename1 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseenCorr.npy'
				sel1 = np.load(filename1)
				filename2 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseenCorr.npy'
				sel2 = np.load(filename2)
			elif ListSelection is 'BothDirUnseenIncorr':
				filename1 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelLeftUnseenIncorr.npy'
				sel1 = np.load(filename1)
				filename2 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-labelRightUnseenIncorr.npy'
				sel2 = np.load(filename2)
			#Load associated labels
			if (analysis is not 'Resp_TrainAll_TestAll') & (analysis is not 'Resp_TrainRot_TestRot'):
				filename = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allall-gatForWeights.p'
			else:
				#filename = path + 'Loc_TrainAll_TestAll/IndRes/' + subject + '_Train_All_Test_Allall-gatForWeights.p' #if estimating angle for resp, we still want to compare it to actual target loc
				filename = path + 'Infer_TrainAll_TestAll/IndRes/' + subject + '_Train_All_Test_Allall-gatForWeights.p' #if estimating angle for resp, we still want to compare it to actual target loc
