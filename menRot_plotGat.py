###Purpose: Basic plotting functions for gat (adapted from J.R. King)
###Project: MenRot
...
def pretty_gat(scores, times=None, chance=0, ax=None, sig=None, cmap='RdBu_r',
               ...
               test_times=None, contourPlot=None, steps=None):
    ...
    #Setup color range
    if clim is None:
        spread = 2 * np.round(np.percentile(np.abs(scores - chance), 99) * 1e2) / 1e2
        m = chance
    ...
    if colorbar:
        pretty_colorbar(im, ax=ax, ticks=[vmin, chance, vmax],
    ...
    ax.set_xticklabels(xticklabels, fontdict={'fontname': 'Arial', 'fontsize': 14})
    yticks, yticklabels = _set_ticks(times)
    ax.set_yticks(yticks)
    ax.set_yticklabels(yticklabels, fontdict={'fontname': 'Arial', 'fontsize': 14})
    if len(xlabel):
        ax.set_xlabel(xlabel, fontdict={'fontname': 'Arial', 'fontsize': 14})
    if len(ylabel):
        ax.set_ylabel(ylabel, fontdict={'fontname': 'Arial', 'fontsize': 14})
...
def pretty_decod(scores, times=None, chance=0, ax=None, sig=None, width=3.,
                 color='k', fill=False, xlabel='Times', sfreq=250, alpha=.75, scat=False, line=True, lim=None, thickness=4,
    ...
    if sig is not None:
        ...
        plt.plot(times, scores_m, color=color)
