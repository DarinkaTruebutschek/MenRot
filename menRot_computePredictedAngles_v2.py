...
for anali, analysis in enumerate(ListAnalysis):
	for subi, subject in enumerate(ListSubjects):
		...
		if ListSelection is 'all':
			...
			filename1 = path + analysis + '/IndRes/' + subject + '_Train_All_Test_Allv-labelLeftUnseen.npy'
			sel1 = np.load(filename1)
		...
		if (ListSelection is not 'BothDirSeen') and (ListSelection is not 'BothDirUnseen'):
			y_pred = y_pred[:, :, sel]
			y_train = y_train[sel]
		else:
			y_pred1 = y_pred[:, :, sel1]
