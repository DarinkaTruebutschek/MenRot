#Purpose: This script computes the distributions of predicted angles from the regression analysis, taking the time course of a specific slice rather than the diagonal.
#Project: MenRot
...
def computePredictedAngles_slice(my_analysis, my_visibility, my_bins, my_split, my_slice):
	...
	ind1 = np.abs(time-my_slice[0]).argmin()
	ind2 = np.abs(time-my_slice[1]).argmin()
	...
	for anali, analysis in enumerate(ListAnalysis):
		for subi, subject in enumerate(ListSubjects):
			...
			if (ListSelection is not 'BothDirSeen') and (ListSelection is not 'BothDirUnseen'):
				...
				#Extract diagonal
				for triali in np.arange(0, np.shape(angle_error)[2]):
					angle_error_diag[triali, :] = np.mean(angle_error[ind1:ind2, :, triali], axis=0)
		...
		if smooth:
			...
			np.save(res_path + ListSelection + '_' + str(len(n_bins)) +  '_Slices_' + str(my_slice[0]) + '_to_' + str(my_slice[1]) + '-histogram.npy', all_hists)